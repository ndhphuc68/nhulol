import {getListHelpRequest, deleteHelpCenterRequest} from "@/api/helpCenter/request"
    ...
    handleUpdateListHelp(state, payload) {
        if (state.listHelp != null && state.listHelp?.length && payload) {
            state.listHelp.forEach(e => {
                const check = payload?.find(val => val.cu_idx === e.cu_idx && val.cu_status !== e.cu_status)
                if (check) {
                    state.check = true
