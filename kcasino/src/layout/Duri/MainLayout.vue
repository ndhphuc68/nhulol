  ...
  <ModalStart
    :image-logo="
      customPage.logo_image
        ? customPage.logo_image
        : require('@/assets/Duri/logo/logo.png')
    "
    v-if="dataPopUp !== 0"
  />
  <div
    class="wrapperDuri-layout"
          ...
          <div class="group-toggle-menu">
            <button v-if="!isOpenSidebar" @click="isOpenSidebar = true">
              <img
                ...
                src="@/assets/Duri/icon/commons/menu.png"
              />
            </button>
            <button class="wrapper-logo" @click="() => router.push('/')">
              <img
                src="@/assets/Duri/logo/logo.png"
                class="logo"
                alt="logo"
                width="70"
                height="70"
...
<script setup>
import Sidebar from "@/components/Duri/Sidebar";
import SidebarHoriontal from "@/components/Duri/Sidebar/SidebarHoriontal.vue";
import Header from "@/components/Duri/Header";
import Footer from "@/components/Duri/Footer";
import { ref } from "vue";
import images from "@/assets/Duri";
import { useRouter } from "vue-router";
import NavbarMobile from "@/components/Duri/NavbarMobile";
import ModalPage from "@/components/Duri/modal/ModalPage";
...
onMounted(async () => {
  ...
  store.commit("handleUpdateLoading", false);
  await store.dispatch("getListGameProviderActionsV7");
});
</script>
<style lang="scss">
.wrapperDuri-layout {
