<template>
  <div :class="layout.nav_pos === '1' && 'v-row w-100' ">
    <div :class="layout.nav_pos === '1' && 'v-col-6 d-flex flex-row align-center'">
      <div :class="layout.nav_pos === '1' && 'v-row d-flex align-center'">
        <div :class="layout.nav_pos === '1' ? 'v-col-4 title-item' :'title-item'">
          <h4 :class="layout.nav_pos === '1' && 'text-center'">{{ $t("dailywithdraw") }}</h4>
        </div>
        <div
          :class="layout.nav_pos === '1' ? 'v-col-8 list-daily-horiontal d-flex align-center justify-space-evenly' :'list-daily'">
          <ul class="list-ul w-100">
            <li
              ...
              :key="item.wi_idx"
            >
              <p class="item-daily-date">{{ formatDateV1(item.wi_created) }}</p>
    ...
    <div :class="layout.nav_pos === '1' && 'v-col-6 d-flex flex-row align-center'">
      <div :class="layout.nav_pos === '1' && 'v-row d-flex align-center'">
        <div :class="layout.nav_pos === '1' ? 'v-col-4 title-item' :'title-item'">
          <h4 :class="layout.nav_pos === '1' && 'text-center'">{{ $t("dailyDeposit") }}</h4>
        </div>
        <div
          :class="layout.nav_pos === '1' ? 'v-col-8 list-daily-horiontal d-flex align-center justify-space-evenly' :'list-daily'">
          <ul class="list-ul w-100">
            <li class="item-daily" v-for="item in dailyDeposits" :key="item.de_idx">
              <p class="item-daily-date">{{ formatDateV1(item.de_created) }}</p>
