  ...
  <div class="container-desktop">
    ...
    <div class="content" v-else>
      <div class="table-data">
        <div class="title-table" v-if="displayData.length !== 0">
          <div
            class="text-center"
            v-for="(item, index) in columnsBetting"
            :key="index"
            style="background-color: #f7f7f9; padding: 13px 0"
          >
            {{ $t(item) }}
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div>
              <span class="amount">
                {{ convertKrTime(item.mgr_date_time) }}
              </span>
            </div>
          </div>
...
<script setup>
import { columnsBetting } from "./data";
...
const handleClickDetail = ({ mgr_row_idx, mgr_game_code, mgr_game_name }) => {
  ...
  if (process.env.VUE_APP_SERVICE_TYPE == "SUN") {
    window.open(
      `https://cms.sunshine789.com/admin/game_detail_history/${mgr_row_idx}/${mgr_game_code}/${mgr_game_name}`,
      "bettingPopup",
      config
    );
  } else if (process.env.VUE_APP_SERVICE_TYPE == "AAT") {
    window.open(
      `https://cms.aat788.com/admin/game_detail_history/${mgr_row_idx}/${mgr_game_code}/${mgr_game_name}`,
      "bettingPopup",
      config
    );
  }
