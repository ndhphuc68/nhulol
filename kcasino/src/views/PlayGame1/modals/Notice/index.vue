<template>
  <div class="deposit-history" v-if="listNotice">
    <h3 class="title-head text-center mb-2">{{$t('notice')}}</h3>
    <EmptyData v-if="listNotice?.length === 0"/>
    <div class="table-notice" v-else>
      <div class="title">
        <div>#</div>
        <div>{{ $t('title') }}</div>
        <div>{{ $t('date') }}</div>
...
import EmptyData from "@/components/PlayGame1/EmptyData/index.vue";
