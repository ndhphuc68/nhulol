...
const handleDelete = (row) => {
  confirm(t("do you really want to delete?"), async () => {
    await handleDeleteMessage(row.mm_idx);
  });
};

const handleDeleteMessage = async (idDelete) => {
  ...
  <tr class="action-row" @click="handleClickRow(row)">
    ...
    <template v-for="(col, key, index) in columns" :key="index">
          ...
          <div class="value">
            <div v-if="!columns[key]?.isStatus">
              <span v-if="columns[key]?.callback">
                {{ columns[key]?.callback(row, row[key]) }} </span
              ><span
                :class="row?.mm_read_date === 0 && 'font-weight-bold'"
                v-else
              >
                {{ row[key] }}</span
              >
      ...
      <td v-if="isViewDetail && cellClickOnMobile === key" class="in-mobile">
        <div class="detail-content d-flex flex-column align-start">
          <h6>{{ row?.mm_subject }}</h6>
          <span class="text-start">{{ row[detailKey] }}</span>
        </div>
      </td>
    </template>
