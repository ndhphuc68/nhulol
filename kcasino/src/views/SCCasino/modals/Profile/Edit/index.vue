...
const emit = defineEmits(["handleChangeEdit"]);
...
const onSubmit = handleSubmit(async (values) => {
  if (!errors.value.password) {
    ...
    res?.is_success && emit("handleChangeEdit");
    ...
    <form @submit.prevent="onSubmit">
      <div class="row">
        ...
        <div class="d-flex justify-end btn-group">
          <button type="submit" class="btn-edit btn-contained" style="background: #0189ff">
            {{ $t("save") }}
          </button>
          <button
            class="btn-cancel btn-hover"
            @click="$emit('handleChangeEdit')"
          >
            {{ $t("cancel") }}
          </button>
        </div>
