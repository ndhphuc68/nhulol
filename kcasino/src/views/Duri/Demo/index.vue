<template>
  <div class="slot-wrapper">
    <div class="slot-top row justify-content-between">
      <div class="row title-slot p-3">
        <div class="d-flex align-center col-12">
          <img
            alt="icon"
            class="icon-slot"
            src="../../../assets/Duri/icon/slot/slot.png"
          />
      ...
      <div class="col-12 my-5">
        <div class="row">
          <div
            ...
            :key="index"
            v-for="(item, index) in listCompanySlot.slice(0, 1)"
          >
            <button
      ...
      <div
        class="col-lg-6 col-12 d-flex justify-content-center logo-img-v1 my-3 d-none"
      >
        <img
          ...
          :src="
            typeGamejs === 'game.js'
              ? require(`@/assets/Duri/logos/companies-game/${
      ...
      <div class="col-lg-6 col-sm-12 d-flex align-center">
        ...
        <Button @click="handleToggleFilter" class="d-none">
          <div class="d-flex align-center">
            <img
              alt="filter"
              class="filter-icon"
              src="../../../assets/Duri/icon/commons/filter.png"
            />
...
<script setup>
import GameItem from "@/views/Duri/Components/GameItem/index.vue";
import Button from "@/components/Duri/customs/Button/index.vue";
import FilterCompany from "@/views/Duri/Components/FilterCompany/index.vue";
...
const gameKey = computed(() => store.state.games7.gameKey);

// const gameKeyV1 = computed(() => store.state.games.gameKey);

const gameKeyV1 = ref(gameTypeModule17.slot[0]);
...
const listGameDetailSlotLose = computed(
  () => store.getters.listGameDetailProviderSlotV7
);
...
const handleCheckCompany = (item) => {return true;
  if (item) {
    if (item.key === 999999) {
...
watch(gameKey, async (value) => {
  ...
  // }
  // if (key) {
  await handleSelectGame(value.key);
  // } else {
...
onMounted(async () => {
  store.commit("handleUpdateGameKeyV7", gameTypeModule17.slot[0]);
  ...
  if (true) {
    ...
    if (!onWeb.value) {
      if (tab) {
        ...
        //   module: key === 999991 ? 7 : moduleGameKey.value,
        // });
        tab?.window.close();
        tab = null;
      } else {
...
const handlePlayGame = async (key, game) => {
  setTimeout(async () => {
    ...
    //   `https://demogamesfree-asia.ppgames.net/gs2c/openGame.do?gameSymbol=${game}&cur=KRW&jurisdiction=KR&lobbyUrl=https%3A%2F%2Fnaver.com&stylename=ab_sunshinegames`
    // );
    // const data = {
    //   provider_id:
    //     typeGamejs.value === "game.js" ? gameKeyV1.value : gameKey.value.key,
    //   game_id: game,
    //   module: key.key === 999991 ? 7 : gameKey.value.module,
    // };
    // console.log(res);
    ...
    // } else {
    const res = `https://demogamesfree-asia.ppgames.net/gs2c/openGame.do?gameSymbol=${game}&cur=KRW&jurisdiction=KR&lobbyUrl=https%3A%2F%2Fnaver.com&stylename=ab_sunshinegames`;
    store.commit("handleUpdateModule", gameKey.value.module);
    setTimeout(async () => {
      store.commit("handleUpdateLoading", false);
      await openGamePopup(res, key.key, gameKey.value.module);
    }, 0);
    // }
  }, 1000);
};
...
const handleSelectGame = async (key) => {
  ...
  if (gameSelect) {
    ...
    store.commit("handleUpdateGameKeyV7", gameSelect);
