...
import SlotGameItem from "@/views/Duri777/Home/Components/PickGame/SlotGame/SlotGameItem";
import FilterCompany from "@/views/Duri777/Home/Components/PickGame/Components/FilterCompany";
import Button from "@/components/Duri777/Custom/Button";
...
const slotgames = ref(gameTypeModule17.slot);
...
const handleSelectGame = async (key) => {
  store.commit("handleUpdateGameKeyV7", key);
  displayGame.value = [];
  game_modals.value = [];
...
<template>
  ...
  <FilterCompany
    :casino="false"
    :company-select="gameKey.key.toString()"
