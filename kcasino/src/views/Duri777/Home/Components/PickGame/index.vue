...
import SlotIcon from "@/assets/Duri777/icon/sidebars/slot";
import CasinoIcon from "@/assets/Duri777/icon/sidebars/casino";
import { gameModule117 } from "@/utils/game/dataGameModule1vs7";
...
const moduleSave = computed(() => store.state.games7.module);
...
const openGamePopup = async (res, key, module) => {
  store.commit("handleUpdateModule", module);
  if (res?.length > 0) {
    store.commit("handleUpdateLoading", false);
    let config =
