    ...
    >
      ...
      <div class="data-content">
        ...
        <div class="data-items">
          <div class="user-name">{{ item.mgr_game_name }}</div>
          <div>
            <span class="amount"> {{ convertMoney(item.mgr_bet_money) }} </span>
          </div>
          <div>
            <span class="amount status available"
              >{{ convertMoney(item.mgr_win_value) }}
...
const handleClickDetail = ({ mgr_row_idx, mgr_game_code, mgr_game_name }) => {
  ...
  if(process.env.VUE_APP_SERVICE_TYPE == 'UT')
  {
    window.open(
  ...
  else if(process.env.VUE_APP_SERVICE_TYPE == 'GBELL')
  {
    window.open(
        `https://cms.gbell88.com/admin/game_detail_history/${mgr_row_idx}/${mgr_game_code}/${mgr_game_name}`,
        "bettingPopup",
        config
    );
  }
