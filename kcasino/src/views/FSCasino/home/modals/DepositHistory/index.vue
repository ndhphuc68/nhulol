<template>
  <section class="text-center">
    <article class="history-top p-2 p-sm-3">
    ...
    <div class="history-bottom">
      <div class="list-day">
        ...
        <div class="text-center">
          <button class="mt-2" @click="handleSearch">{{ $t("search") }}</button>
        </div>
      </div>
      <div class="table-wrapper d-none my-3 d-lg-block">
        <DataTable :value="listDeposit" paginator :rows="5" dataKey="index">
      ...
      <div class="table-mobile mt-3 d-lg-none" id="body-table">
        <article class="mt-3" v-for="(item, index) in displayData" :key="index">
          <div>
            <span> No.{{ index + 1 }} </span>
            <span :class="item.de_status">{{ $t(item.de_status) }}</span>
          </div>
          <div class="mt-3">
          ...
          <div class="mt-3">
            ...
            <span>{{ item.de_member_account }}</span>
          </div>
          <div class="mt-3">
      ...
      <div class="mt-3 d-lg-none wrapper-pagination">
        <v-pagination
          v-model="currentPage"
...
const getListDeposit = computed(() => store.state.deposit.listDeposit);
const listDeposit = ref(getListDeposit?.value);
...
const handleSearch = () => {
  if (!dateStart.value || !dateEnd.value || dateEnd.value < dateStart.value)
    return;
  listDeposit.value = getListDeposit?.value.filter(
    (item) =>
...
.history-top {
  background: url("@/assets/FSCasino/bg-deposit-history.png") no-repeat;
