...
const withdrawal = computed(() => store.state.user.userInfor.me_bank_account);
...
const WithdrawalAction = async () => {
  ...
  if (accountHolder?.value.replace(/\s/g, "").length > 5 || accountHolder?.value.replace(/\s/g, "").length < 2) {
    await store.dispatch("showAlert", {
      type: "error",
  ...
  <div class="container">
    <div class="card">
      ...
      <div class="d-flex justify-content-end">
        <Button class="mr-2" variant="contained" @click="WithdrawalAction">
          {{ $t("withdraw") }}
        </Button>
