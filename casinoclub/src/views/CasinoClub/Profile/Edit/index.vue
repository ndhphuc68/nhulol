...
import InputCheck from "@/components/CasinoClub/Input/InputCheck";
...
const { handleSubmit, useFieldModel, errors, setFieldValue, setFieldError } =
  useForm({
    ...
    initialValues: {
      nickName: auth?.value?.me_nickname,
      phone: auth?.value?.me_phone,
      bankName: auth?.value?.me_bank_name,
      bankAccount: auth?.value?.me_bank_account,
      bankNumber: auth?.value?.me_bank_number,
      password: "",
      confirmPassword: "",
    },
  });
...
const onSubmit = handleSubmit(async (values) => {
  if (!errors.value.password) {
    ...
    const data = {
      ...
      NickName:
        values.nickName !== auth?.value?.me_nickname
          ? values.nickName
          : undefined,
...
.edit-info {
  ...
  .btn-group {
    ...
    .btn-save {
      ...
      border-radius: 20px;
      padding: 6px 20px;
      background-color: var(--primaryCasinoClub);
      color: #fff;
      margin-left: 10px;
    }
